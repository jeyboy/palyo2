...
int MediaPlayer::durationMillis() {
   return item_duration / 1000;
}

int64_t MediaPlayer::positionMicro() {
    return clock -> position() * AV_TIME_BASE;
}

int MediaPlayer::positionMillis() {
    return clock -> position() * 1000;
}
////////////////////////////////////
...
void MediaPlayer::seekMicro(int64_t microMillis) {
    if (decoder == 0) return;
...
//        custom_context = new CustomContext(context, url);
//        if (!custom_context -> isValid()) {
//            emit errorOccured(custom_context -> lastError());
//            closeContext();
//        }
//        return custom_context -> isValid();
    } else {
