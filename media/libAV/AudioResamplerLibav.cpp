...
class AudioResamplerLibav : public AudioResampler //Q_AV_EXPORT is not needed
{
...
bool AudioResamplerLibav::convert(const quint8 **data)
{
    ...
    d.out_samples_per_channel = av_rescale_rnd(
                swr_get_delay(d.context, d.in_samples_per_channel) +
    ...
    int converted_samplers_per_channel = swr_convert(d.context, out, d.out_samples_per_channel, const_cast<const uint8_t**>(data), d.in_samples_per_channel);
...
//TODO: why crash if use channel mapping for L or R?
    ...
    if (remix && matrix) {
        swr_set_matrix(d.context, matrix, in_c);
